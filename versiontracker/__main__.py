...
from versiontracker.utils import setup_logging, check_dependencies, get_json_data
...
def main() -> int:
    ...
    try:
        ...
        if options.apps:
            ...
            # Filter out blacklisted apps
            apps_folder = [
                app for app in apps_folder 
                if not config.is_blacklisted(app[0])
            ]
            ...
            if brew_options:
                ...
                if strict_mode:
                    print("No new applications found that can be installed with Homebrew.")
                else:
