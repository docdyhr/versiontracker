...
def parse_version(version_string: Optional[str]) -> Optional[Tuple[int, ...]]:
    ...
    cleaned = re.sub(r'^[a-zA-Z]+\s+(?=\d)', '', cleaned)  # Remove other app names before numbers
    ...
    if prerelease_match and not is_mixed_format:
        ...
        cleaned = re.sub(r'[-.](?:alpha|beta|rc|final|[αβγδ])(?:\.\w+|\.\d+)?.*$', '', cleaned, flags=re.IGNORECASE)
...
def compare_versions(version1: Union[str, Tuple[int, ...], None], version2: Union[str, Tuple[int, ...], None]) -> int:
    ...
    if isinstance(version1, str) and isinstance(version2, str):
        ...
        has_app_name_v1 = bool(re.search(r'^(?:Google\s+)?(?:Chrome|Firefox|Safari)\s+', v1_str, re.IGNORECASE) or 
                               re.search(r'^[a-zA-Z]+\s+(?=\d)', v1_str))
        has_app_name_v2 = bool(re.search(r'^(?:Google\s+)?(?:Chrome|Firefox|Safari)\s+', v2_str, re.IGNORECASE) or 
                               re.search(r'^[a-zA-Z]+\s+(?=\d)', v2_str))
        ...
        if has_app_name_v1 or has_app_name_v2:
            ...
            def normalize_app_version(v_str):
                ...
                cleaned = re.sub(r'^(?:Google\s+)?(?:Chrome|Firefox|Safari)\s+', '', v_str, flags=re.IGNORECASE)
            ...
            if v1_norm != v2_norm and not (_is_prerelease(v1_str) or _is_prerelease(v2_str)):
                ...
                if (v1_parts[:min_len] == v2_parts[:min_len] and 
                    len(v1_parts) != len(v2_parts)):
    ...
    v1_base = v1_padded[:3] if len(v1_padded) >= 3 else v1_padded + (0,) * (3 - len(v1_padded))
    v2_base = v2_padded[:3] if len(v2_padded) >= 3 else v2_padded + (0,) * (3 - len(v2_padded))
...
def _is_prerelease(version_str: str) -> bool:
    ...
    if isinstance(version_str, tuple):
        return False
    return bool(re.search(r'[-.](?:alpha|beta|rc|final|[αβγδ])(?:\W|$)', str(version_str), re.IGNORECASE))
...
def _extract_prerelease_info(version_str: str) -> Tuple[str, Union[int, str, None]]:
    ...
    match = re.search(r'[-.](?P<type>alpha|beta|rc|final|α|β)(?:\.(?P<suffix>\w+|\d+))?', version_str, re.IGNORECASE)
    if match:
...
def get_partial_ratio_scorer():
    ...
    if USE_RAPIDFUZZ and fuzz and hasattr(fuzz, 'partial_ratio'):
        ...
        def fallback_scorer(s1, s2):
            ...
            return 100.0 if s1.lower() == s2.lower() else (70.0 if s1.lower() in s2.lower() or s2.lower() in s1.lower() else 0.0)
        return fallback_scorer


def get_version_difference(version1: Union[str, Tuple[int, ...], None], version2: Union[str, Tuple[int, ...], None]) -> Optional[Tuple[int, ...]]:
...
def get_version_info(current_version: Optional[str], latest_version: Optional[str] = None) -> ApplicationInfo:
    ...
    if latest_version is None:
        ...
        if comparison == 0:
            ...
            app_info.outdated_by = get_version_difference(current_version, latest_version)
        else:
...
def _parse_version_to_dict(version_string: str) -> Dict[str, Union[str, int, Tuple[int, ...], None]]:
