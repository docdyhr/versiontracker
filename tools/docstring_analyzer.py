...
def analyze_directory(directory: Path, exclude_dirs: List[str] = None) -> Dict[Path, List[DocstringInfo]]:
    ...
    for file_path, file_results in sorted(results.items()):
        ...
        for result in file_results:
            ...
            if problems or show_all:
                file_lines.append(f"  Line {result.lineno}: {result.name}: {', '.join(problems) if problems else 'OK'}")
...
def main():
    ...
    parser.add_argument("--all", action="store_true", help="Show all results, not just problems")
    ...
    exclude_dirs = ['__pycache__', '.git', '.github', 'venv', '.venv', 'env'] + args.exclude
