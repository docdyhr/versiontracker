...
async def test_search_casks_with_mock_server(mock_server, server_url):
    ...
    # Configure the base URL to use our mock server
    with patch("versiontracker.async_homebrew.HOMEBREW_SEARCH_BASE", f"{server_url}/api/search"):
        # Search for casks
...
def test_async_check_brew_update_candidates_with_mock_server(mock_server, server_url):
    ...
    with patch("versiontracker.async_homebrew.HOMEBREW_API_BASE", f"{server_url}/api/cask"):
        with patch("versiontracker.async_homebrew.is_homebrew_available", return_value=True):
            ...
            assert nonexistent_result[3] is None  # NonExistentApp should have no version
...
class TestAsyncIntegrationScenarios:
    ...
    def test_batch_processing_with_server_errors(self, mock_server, server_url):
        ...
        with patch.object(mock_server.server.RequestHandlerClass, "do_GET", alternating_error_get):
    ...
    def test_high_concurrency_with_rate_limiting(self, mock_server, server_url):
        ...
        with patch("versiontracker.async_homebrew.HOMEBREW_API_BASE", f"{server_url}/api/cask"):
            with patch("versiontracker.async_homebrew.is_homebrew_available", return_value=True):
                ...
                data = [
                    (f"App{i}", f"1.{i}") for i in range(20)
                ]
                ...
                with patch.object(HomebrewBatchProcessor, "_check_exact_match", mock_check_exact_match):
                    with patch("versiontracker.async_homebrew.fetch_cask_info", track_concurrent_fetch):
