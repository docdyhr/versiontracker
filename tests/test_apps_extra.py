...
class TestAppsExtra(unittest.TestCase):
    ...
    def test_check_brew_install_candidates_success(self):
        ...
        with (
            ...
            patch("versiontracker.apps.finder._process_brew_batch") as mock_process_brew_batch,
            patch("versiontracker.apps.finder.smart_progress") as mock_smart_progress,
        ):
    ...
    def test_process_brew_batch_with_adaptive_rate_limiting(self):
        ...
        with (
            ...
            patch("versiontracker.apps.finder._AdaptiveRateLimiter") as mock_rate_limiter_class,
            patch("concurrent.futures.as_completed") as mock_as_completed,
            patch("versiontracker.apps.finder.get_config") as mock_get_config,
        ):
    ...
    @patch("versiontracker.apps.finder.is_homebrew_available", return_value=True)
    @patch("versiontracker.apps.finder.get_homebrew_casks", return_value=["firefox", "chrome"])
    ...
    @patch("versiontracker.apps.finder.is_homebrew_available", return_value=True)
    @patch("versiontracker.apps.finder.get_homebrew_casks", return_value=["firefox", "chrome", "python", "node"])
    ...
    @patch("versiontracker.apps.finder.is_homebrew_available", return_value=True)
    @patch("versiontracker.apps.finder.get_homebrew_casks", side_effect=HomebrewError("Failed to get Homebrew casks"))
    ...
    @patch("versiontracker.apps.finder.is_homebrew_available", return_value=True)
    @patch("versiontracker.apps.finder.get_homebrew_casks", side_effect=BrewPermissionError("Permission denied"))
    ...
    def test_is_brew_cask_installable_cached(self):
        """Test is_brew_cask_installable with cached data."""
        import versiontracker.apps.finder as finder_module

        with (
            patch.object(finder_module, "is_homebrew_available", return_value=True),
            patch.object(finder_module, "read_cache", return_value={"installable": ["firefox", "chrome"]}),
            patch.object(finder_module, "run_command", return_value=("No formulae or casks found", 1)),
            patch.object(finder_module, "write_cache"),
        ):
    ...
    def test_is_brew_cask_installable_found(self):
        ...
        with (
            patch("versiontracker.apps.finder.is_homebrew_available", return_value=True),
            patch("versiontracker.apps.finder.read_cache", return_value=None),
            patch("versiontracker.apps.finder.run_command", return_value=("firefox", 0)),
            patch("versiontracker.apps.finder.write_cache") as mock_write_cache,
        ):
            # Call the function
    ...
    def test_is_brew_cask_installable_not_found(self, mock_run_command, mock_read_cache, mock_is_homebrew):
        ...
        # Patch is_homebrew_available in the finder module too
        with patch.object(finder_module, "is_homebrew_available", return_value=True):
            # Call the function
