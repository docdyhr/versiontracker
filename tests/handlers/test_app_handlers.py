...
def test_handle_list_apps_success(
    mock_stdout, mock_get_config, mock_get_applications, mock_get_json_data
):
    ...
    assert "Found 2 applications" in output
...
def test_handle_list_apps_with_blacklist(
    ...
    output = mock_stdout.getvalue()
    assert "Found 2 applications" in output
    assert "BlacklistedApp" not in output
...
def test_handle_list_apps_with_export(
    ...
    assert mock_handle_export.call_args[0][0] == expected_data
    assert mock_handle_export.call_args[0][1] == "json"
...
@patch("versiontracker.handlers.app_handlers.get_config")
@patch("sys.stdout", new_callable=StringIO)
def test_handle_list_apps_error(mock_stdout, mock_get_config, mock_get_json_data):
